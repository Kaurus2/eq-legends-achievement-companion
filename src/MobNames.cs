...
public partial class MainWindow {
 ...
 void BuildMobNames(StackPanel parent){
  ...
  section.Children.Add(Text("Mob names",15));mobNameRows=new StackPanel();section.Children.Add(new ScrollViewer{Content=mobNameRows,MaxHeight=160,VerticalScrollBarVisibility=ScrollBarVisibility.Auto});
  ...
  mobNameInput=Input();mobNameInput.ToolTip="Enter a mob name";mobNameInput.MaxLength=160;entry.Children.Add(mobNameInput);
  var add=Button("+",AddMobName);add.ToolTip="Add mob name";Grid.SetColumn(add,1);entry.Children.Add(add);section.Children.Add(entry);
  mobNameInput.KeyDown+=(s,e)=>{if(e.Key==Key.Enter){AddMobName();e.Handled=true;}};RefreshMobNames();
 }
 ...
 void ChangeMobName(string name,bool add){
  ...
  if(add){if(names.Any(n=>String.Equals(n,name,StringComparison.OrdinalIgnoreCase))){status.Text="That mob name is already listed.";return;}names.Add(name);}else names.RemoveAll(n=>String.Equals(n,name,StringComparison.OrdinalIgnoreCase));
