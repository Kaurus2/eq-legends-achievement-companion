...
public partial class MainWindow {
 ...
 void BuildProfileControls(Panel toolbar){
  ...
  var setupInfo=Button("ⓘ",()=>{setupHelp.Visibility=setupHelp.Visibility==Visibility.Visible?Visibility.Collapsed:Visibility.Visible;});setupInfo.ToolTip="How to enable logging and export achievements";actions.Children.Add(setupInfo);actions.Children.Add(Button("Setup guide",OpenSetup));panel.Children.Add(actions);panel.Children.Add(setupHelp);
 ...
 void UpdateConnections(){if(connectedStatus==null)return;if(ActiveProfile==null&&settings.Profiles.Count>1){connectedStatus.Text="Select your character in the top dropdown to check its files.";connectedStatus.ToolTip="Multiple characters were found. Choose which one to track.";return;}bool export=File.Exists(source.Text),log=File.Exists(combatPath.Text);connectedStatus.Inlines.Clear();connectedStatus.Inlines.Add(new System.Windows.Documents.Run(export?"✓":"✕"){Foreground=export?System.Windows.Media.Brushes.LimeGreen:System.Windows.Media.Brushes.IndianRed});connectedStatus.Inlines.Add(export?" Export connected · ":" Export needed · ");connectedStatus.Inlines.Add(new System.Windows.Documents.Run(log?"✓":"✕"){Foreground=log?System.Windows.Media.Brushes.LimeGreen:System.Windows.Media.Brushes.IndianRed});connectedStatus.Inlines.Add(log?" Combat log connected":" Combat log needed");connectedStatus.ToolTip=(export?source.Text:"Export achievements in game, then Find characters.")+"\n"+(log?combatPath.Text:"Use /log on in game, then Find characters.");}
 ...
 void ScanProfiles(){try{if(!ResolveEdits())return;var discovered=DiscoverProfiles(gameFolder.Text.Trim());RememberProfile();settings.GameFolder=Path.GetFullPath(gameFolder.Text.Trim());MergeProfiles(discovered);Data.Save(settingsPath,settings);RefreshProfilePicker();UpdateConnections();profileStatus.Text=discovered.Count+" characters found. "+(ActiveProfile==null&&discovered.Count>1?"Choose your character in the top dropdown.":"Missing exports: export achievements in game. Missing logs: use /log on.");if(ActiveProfile!=null)SwitchProfile(ActiveProfile,true);else if(discovered.Count==1)SwitchProfile(settings.Profiles.First(p=>p.Key==discovered[0].Key));}catch(Exception e){profileStatus.Text="Could not scan folder: "+e.Message;}}
