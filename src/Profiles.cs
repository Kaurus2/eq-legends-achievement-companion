...
public partial class MainWindow {
 ...
 void BuildProfileControls(Panel toolbar){
  ...
  var actions=new WrapPanel();actions.Children.Add(Button("Browse folder…",()=>{using(var d=new System.Windows.Forms.FolderBrowserDialog()){d.Description="Choose your EverQuest Legends installation folder";d.SelectedPath=gameFolder.Text;if(d.ShowDialog()==System.Windows.Forms.DialogResult.OK){gameFolder.Text=d.SelectedPath;ScanProfiles();}}}));actions.Children.Add(Button("Find characters",ScanProfiles));panel.Children.Add(actions);
 ...
 void UpdateConnections(){if(connectedStatus==null)return;bool export=File.Exists(source.Text),log=File.Exists(combatPath.Text);connectedStatus.Text=(export?"✓ Export connected":"Export needed")+" · "+(log?"✓ Combat log connected":"Logging needed");connectedStatus.ToolTip=(export?source.Text:"Export achievements in game, then Find characters.")+"\n"+(log?combatPath.Text:"Use /log on in game, then Find characters.");}
 ...
 void SwitchProfile(CharacterProfile p,bool reload=false){
  ...
  live=new LiveMonitor(settings.Live,()=>achievements,()=>settings.SourcePath+loadedHash+loadedExportStamp);live.Notified+=LiveProgress;live.ResetProgress+=ClearLiveTiles;RefreshProfilePicker();
 }
