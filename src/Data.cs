...
public static class Data {
 ...
 public static bool ApplyResearchUpdates(List<Achievement> all,ResearchFile file){
  const string id="2026-09-10-locations-and-camp-observations";if(file.AppliedUpdates==null)file.AppliedUpdates=new List<string>();if(file.AppliedUpdates.Contains(id))return false;
  ...
  foreach(string name in new[]{"Oh the Humanity!","Highly Uncivilized","Barbarous","Doesn't Play Well With Others","I'm a People Person!"})add(name,new Farm{Zone="West Karana",Region="Karanas",Mobs="Bandits — Human, High Elf and Barbarian variants (select the needed race)",Faction="No faction hit observed (user)",Evidence="User EQL observation, 2026-09-10: West Karana bandits include Humans, High Elves and Barbarians. Applies to the observed targets, not all bandits worldwide."});
