...
def start(title,sub):
 ...
 c.setFillColor(blue);c.setFont('Helvetica',10);c.drawString(40,22,'EQ Legends Achievement Companion | v2026.09.17.2');c.drawRightString(920,22,str(page))
...
y=250
for text in ['Popups slide down and fade in. Progress popups stay eight seconds; completion popups stay thirteen seconds, with a gold border and a longer victory tune. When one expires, it fades out and the others slide up.', 'Your own kills and recognized pet kills produce estimates. Group kills can also count when a nearby party-experience message shares the kill timestamp. Generic bandit names do not identify a race.', 'Periodically make a fresh achievement export. The app detects it and updates the confirmed counts; you can also use Load / Refresh under Advanced file settings.', 'A popup can appear for an achievement hidden by your filters. It will not override your filters to insert that tile.']:
...
y=655
for text in ['A kill-count Slayer tile starts with Learning pace. After at least five qualifying kills over 30 seconds, it shows an estimate such as ~12 min left.', 'The estimate uses your kills from the last five minutes and divides the estimated remaining count by that pace. It changes as you speed up or slow down.', 'After 90 seconds without a qualifying kill, the tile shows Paused. When kills resume after that gap, it learns a new pace.', 'Open the achievement for the fuller breakdown: estimated kills left, kills per minute and estimated time. Hover over the tile estimate for the same detail.', 'Names you save in Mob names contribute to this estimate when the logged kill matches. Group kills require a nearby party-experience message with the same timestamp; recognized pet kills can count too.', 'The estimate covers killing time. Travel, finding a camp, turn-ins and other setup are not included.', 'A fresh achievement export replaces estimates with confirmed game progress and starts a new pace sample. Reaching an estimated target is not proof of completion.', 'With State set to Incomplete, confirmed completed achievements leave the results. Choose All states or Complete to view finished achievements.']:
 y=para(text,40,y,105,15)
